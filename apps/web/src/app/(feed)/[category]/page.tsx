...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  // TODO: Fetch articles filtered by category from DB
  // const articles = await db.query.articles.findMany({ where: eq(articles.categorySlug, category), ... });
      ...
      <header className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)] mt-1">
            AI-Powered News &middot;{' '}
            <span
              className="font-medium"
              style={{ color: activeCategory?.color ?? 'var(--accent-cyan)' }}
            >
              {categoryName}
            </span>
          </p>
